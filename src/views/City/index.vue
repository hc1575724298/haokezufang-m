 ...
 * @LastEditors: sj
 * @LastEditTime: 2022-07-20 14:35:10
-->
<template>
  <div class="city">
    ...
    >
      ...
      <van-index-anchor
        :index="item"
        v-for="item in indexList.slice(2)"
        :key="item.label"
      >
        <span> {{ item }}</span>
        <div v-if="filterCityList[item]?.length > 0">
          <van-cell
            :title="city"
            v-for="(city, index) in filterCityList[item]"
            :key="index"
  ...
  methods: {
    async getCity() {
      try {
        ...
        const obj = {}
        this.indexList.slice(2).forEach((ele) => {
          obj[ele] = []
        })
    ...
    async changeCity(val) {
      try {
        const {
          data: { body }
        } = await getCityInfo(val)
        console.log(body)

        // 获取到选择的城市， 把城市信息存入本地
        // localStorage.setItem('cityInfo', JSON.stringify(body))
        this.$store.commit('getCity', body)
        // const res = await getCityHouseInfo(body.value)
        // console.log(res)
        this.$router.push({
          path: '/layout/home'
          // query: {
          //   nameCity: JSON.parse(localStorage.getItem('cityInfo')).label
          // }
        })
      } catch (error) {
        console.log(error)
